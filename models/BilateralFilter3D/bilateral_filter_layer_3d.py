...
class BilateralFilter3d(torch.nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        input_tensor = input_tensor.permute(0, 1, 2, 4, 5, 3).contiguous().view(B*T, C, X, Y, Z)

        if self.use_gpu:
            output = BilateralFilterFunction3dGPU.apply(input_tensor,
                                                      self.sigma_x,
                                                      self.sigma_y,
                                                      self.sigma_z,
                                                      self.color_sigma)
