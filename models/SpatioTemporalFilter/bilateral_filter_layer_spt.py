...
class BilateralFilterFunction4dsptGPU(torch.autograd.Function):
    """
    ...
    @staticmethod
    def forward(ctx, input_img, sigma_x, sigma_y, sigma_z, sigma_t, color_sigma):
        ...
        outputTensor, outputWeightsTensor, dO_dx_ki, dO_dsig_r, dO_dsig_x, dO_dsig_y, dO_dsig_z, d0_dsig_t = bilateralfilter4dspt_gpu_lib.forward_4dspt_gpu(input_img, sigma_x, sigma_y, sigma_z, sigma_t, color_sigma)
    ...
    def backward(ctx, grad_output):
        ...
        outputTensor = ctx.saved_tensors[6]  # filtered image
        outputWeightsTensor = ctx.saved_tensors[7]  # weights
        dO_dx_ki = ctx.saved_tensors[8]  # derivative of output with respect to input, while k==i
        dO_dsig_r = ctx.saved_tensors[9]  # derivative of output with respect to range sigma
        dO_dsig_x = ctx.saved_tensors[10]  # derivative of output with respect to sigma x
        dO_dsig_y = ctx.saved_tensors[11]  # derivative of output with respect to sigma y
        dO_dsig_z = ctx.saved_tensors[12]  # derivative of output with respect to sigma z
        d0_dsig_t = ctx.saved_tensors[13] # derivative of output with respect to sigma t
        ...
        grad_output_tensor = bilateralfilter4dspt_gpu_lib.backward_4dspt_gpu(grad_output,
                                                                     input_img,
                                                                     outputTensor,
                                                                     outputWeightsTensor,
                                                                     dO_dx_ki,
        ...
        return grad_output_tensor, grad_sig_x, grad_sig_y, grad_sig_z, grad_sig_t, grad_color_sigma


class BilateralFilter4dspt(torch.nn.Module):
    def __init__(self, sigma_x, sigma_y, sigma_z, sigma_t, color_sigma, pad_t=4, use_gpu=True):
    ...
    def _get_sigmas(self):
        return self.sigma_x, self.sigma_y, self.sigma_z, self.sigma_t, self.color_sigma
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        if self.use_gpu:
            output = BilateralFilterFunction4dsptGPU.apply(input_tensor,
                                                      self.sigma_x,
                                                      self.sigma_y,
                                                      self.sigma_z,
                                                      self.sigma_t,
