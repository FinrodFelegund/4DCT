"""
Gradcheck for the joint spatio-temporal bilateral filter layer (bilateral_filter_layer_spt), GPU only.
Documentation: https://pytorch.org/docs/stable/generated/torch.autograd.gradcheck.html
"""
from bilateral_filter_layer_spt import BilateralFilterFunction4dsptGPU
import torch
from torch.autograd import gradcheck

SHAPE = (1, 1, 6, 8, 8, 8)  # [B, C, T, X, Y, Z]
VALUES = {'sigma_x': 1.1, 'sigma_y': 1.1, 'sigma_z': 1.1, 'sigma_t': 1.1, 'color_sigma': 0.5}
...
def gradient_sigma(layer_bf, name, eps, atol):
    tensor_in = torch.randn(SHAPE, dtype=torch.double, device='cuda')
    print(gradcheck(layer_bf, (tensor_in, *sigmas(name)), eps=eps, atol=atol))


layer_bf_gpu = BilateralFilterFunction4dsptGPU.apply
...
for sigma_name in ['sigma_x', 'sigma_y', 'sigma_z', 'sigma_t']:
    ...
    gradient_sigma(layer_bf_gpu, sigma_name, eps=1e-2, atol=1e-3)

print('Gradient with respect to sigma_range:')
gradient_sigma(layer_bf_gpu, 'color_sigma', eps=1e-3, atol=1e-3)
