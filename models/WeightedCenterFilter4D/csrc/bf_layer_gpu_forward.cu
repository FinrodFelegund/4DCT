...
__constant__ float cCenter_alpha;


template <typename scalar_t, int C>
__global__ void BilateralFilterCudaKernel4DWeightedCenterForward(scalar_t* input,
                                                   scalar_t* output,
  ...
  scalar_t centerWeight_w = 0;
  scalar_t centerWeight_alpha = 0; 
  ...
  #pragma unroll
          ...
          int halfWindowT = cHalfWindowSize_arr[0];
          int halfWindowX = cHalfWindowSize_arr[1];
          int halfWindowY = cHalfWindowSize_arr[2];
          int halfWindowZ = cHalfWindowSize_arr[3];

          bool isCenter = (kernelT == halfWindowT) && (kernelX == halfWindowX) && (kernelY == halfWindowY) && (kernelZ == halfWindowZ);
          ...
          scalar_t rawWeight = spatialWeight * colorWeight;
          scalar_t totalWeight = 0;
          if(isCenter){
            totalWeight = cCenter_alpha * rawWeight;
          } else {
  ...
  #pragma unroll
              for (int c = 0; c < C; c++) {
                  ...
                  if(isCenter){
                    centerWeight_w += rawWeight;
                    centerWeight_alpha += rawWeight * input[batchOffset + homeOffset + c * cColorStride];
                  }
...
//    output[batchOffset + homeOffset + c * cColorStride] /= weightSum;
    ...
    dO_dx_ki[batchOffset + homeOffset + c * cColorStride] =
          ...
          dw_dx_ki +
          (1 / weightSum) * (dfilter_dx_ki + cCenter_alpha * centerWeight_w); // +1 now becomes d(w_ix_1)/dx_k
    ...
    d0_dcenter_alpha[batchOffset + homeOffset + c * cColorStride] =
          -(1 / weightSum) * (valueSum / weightSum) *
          centerWeight_w + (1 / weightSum) * centerWeight_alpha;
  }
}
...
void BilateralFilterCudaForwardFunction(torch::Tensor inputTensor,
  ...
  cudaMemcpyToSymbol(cCenter_alpha, &center_alpha, sizeof(float));
...
#define BLOCK_SIZE 32
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      inputTensor.scalar_type(), "BilateralFilterCudaKernel4DWeightedCenterForward", ([&] {
                BilateralFilterCudaKernel4DWeightedCenterForward<scalar_t, C>
                <<<dim3(int(desc.channelStride / BLOCK_SIZE) + 1, desc.batchCount), dim3(BLOCK_SIZE, 1)>>>(
                    ...
                    d0_dcenter_alpha.data_ptr<scalar_t>());
        }
      ));
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> BilateralFilterCudaForward(torch::Tensor inputTensor,
                                                                                                                                               float sigma_x,
                                                                                                                                               float sigma_y,
                                                                                                                                               float sigma_z,
                                                                                                                                               float sigma_t,
  ...
  torch::Tensor d0_dcenter_alpha = torch::zeros_like(inputTensor);
  cuda_error_check("beginning");

#define CASE(c, d) BilateralFilterCudaForwardFunction<c, d>(inputTensor, outputTensor, outputWeightsTensor, dO_dx_ki, dO_dsig_r, dO_dsig_x, dO_dsig_y, dO_dsig_z, d0_dsig_t, d0_dcenter_alpha, sigma_x, sigma_y, sigma_z, sigma_t, colorSigma, center_alpha);
  SWITCH_AB(CASE, BF_CUDA_MAX_CHANNELS, BF_CUDA_MAX_SPATIAL_DIMENSION, inputTensor.size(1), inputTensor.dim() - 2);

  return {outputTensor, outputWeightsTensor, dO_dx_ki, dO_dsig_r, dO_dsig_x, dO_dsig_y, dO_dsig_z, d0_dsig_t, d0_dcenter_alpha};
}
