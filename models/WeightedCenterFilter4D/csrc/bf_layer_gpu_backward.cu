...
__global__ void BilateralFilterCudaKernel4DsptBackward(scalar_t* gradientInputTensor,
  ...
  #pragma unroll
              for (int c = 0; c < C; c++) {
                  ...
                  if (kernelT != cHalfWindowSize_arrBack[0] || kernelX != cHalfWindowSize_arrBack[1] || kernelY != cHalfWindowSize_arrBack[2] ||
                      kernelZ != cHalfWindowSize_arrBack[3]) {
...
#define BLOCK_SIZE 32
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      inputTensor.scalar_type(), "BilateralFilterCudaKernel4DsptBackward", ([&] {
            BilateralFilterCudaKernel4DsptBackward<scalar_t, C>
...
#define CASE(c, d) BilateralFilterCudaBackwardFunction<c, d>(gradientInputTensor, gradientOutputTensor, inputTensor, outputTensor, outputWeightsTensor, dO_dx_ki, sigma_x, sigma_y, sigma_z, sigma_t, colorSigma);
