...
class CTCacheDataset(Dataset):
    ...
    @staticmethod
    def get_cache_transforms(destination: str, max_proj: float):
        ...
        transforms = Compose([
            ...
            GenerateNoiseScans(keys=clean_keys, dosage=0.25, pixel_size_mm=PIXEL_SIZE_IN_MM, max_proj=max_proj),
...
class CTValidationDataset(Dataset):
    ...
    def get_validation_transforms():
        ...
        transforms = Compose([
            ...
            ForegroundBBox(keys=clean_keys + noise_keys),
...
def cacheDataSet():
    DATASETS = ['/home/dpietsch/Data/Inhouse2']#, '/home/dpietsch/Data/Inhouse1']
    cache_dest = '/home/dpietsch/Data/.cache'
    Path(cache_dest).mkdir(parents=True, exist_ok=True)
    DESTINATIONS = [f'{cache_dest}/Inhouse2Processed']


    max_list = []
    for dataset, destination in zip(DATASETS, DESTINATIONS):
        dataframe = CTCacheDataset.generate_dataframe(dataset)
        cache_transforms = CTCacheDataset.get_cache_transforms(destination=destination, max_proj=128.98109436035156)
        ...
        for index, out in enumerate(tqdm_loader):
            pass
            #tqdm_loader.set_description(f'Cached batch with index {index}, scans: {keys}, shapes: {max_proj}')
